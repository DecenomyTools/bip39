(function () {
  ...
  DOM.entropyTypeInputs = DOM.entropyContainer.find(
    "input[name='entropy-type']"
  );
  ...
  DOM.entropyWeakEntropyOverrideWarning = DOM.entropyContainer.find(
    ".weak-entropy-override-warning"
  );
    ...
    if (isUsingAutoCompute()) {
      ...
      phraseChangeTimeoutEvent = setTimeout(function () {
        ...
        if (entropy !== null) {
          ...
          DOM.entropyTypeInputs
            .filter("[value='hexadecimal']")
            .prop("checked", true);
  ...
  function calcBip32RootKeyFromBase58(rootKeyBase58) {
    ...
    if (networkHasSegwit()) {
      ...
      if ("p2wpkh" in n) {
        try {
          bip32RootKey = libs.bitcoin.HDNode.fromBase58(
            rootKeyBase58,
            n.p2wpkh
          );
          return;
  ...
  function calcBip32RootKeyFromBase58GRS(rootKeyBase58) {
    ...
    if (networkHasSegwit()) {
      ...
      if ("p2wpkh" in n) {
        try {
          bip32RootKey = libs.groestlcoinjs.HDNode.fromBase58(
            rootKeyBase58,
            n.p2wpkh
          );
          return;
  ...
  function findDerivationPathErrors(path) {
    ...
    var hardenedAddresses =
      bip32TabSelected() && DOM.hardenedAddresses.prop("checked");
    ...
    function calculateValues() {
      setTimeout(function () {
        ...
        if (networks[DOM.network.val()].name == "HNS - Handshake") {
          var ring = libs.handshake.KeyRing.fromPublic(
        ...
        // Bitcoin Cash address format may vary
        if (
          networks[DOM.network.val()].name == "SLP - Simple Ledger Protocol"
        ) {
          var bchAddrType = DOM.bitcoinCashAddressType.filter(":checked").val();
          if (bchAddrType == "cashaddr") {
        ...
        if (isSegwit) {
          ...
          if (isP2wpkh) {
            ...
            var scriptpubkey =
              libs.bitcoin.script.witnessPubKeyHash.output.encode(keyhash);
            address = libs.bitcoin.address.fromOutputScript(
              scriptpubkey,
              network
            );
            ...
            var scriptpubkey =
              libs.bitcoin.script.scriptHash.output.encode(addressbytes);
            address = libs.bitcoin.address.fromOutputScript(
              scriptpubkey,
              network
            );
            ...
            // This is a 1-of-1
            var witnessScript = libs.bitcoin.script.multisig.output.encode(1, [
              key.getPublicKeyBuffer(),
            ]);
            var scriptPubKey =
              libs.bitcoin.script.witnessScriptHash.output.encode(
            ...
            var scriptPubKey = libs.bitcoin.script.scriptHash.output.encode(
        ...
        if (isGRS()) {
          if (isSegwit) {
            ...
            if (isP2wpkh) {
              address = libs.groestlcoinjs.address.fromOutputScript(
                scriptpubkey,
                network
              );
            } else if (isP2wpkhInP2sh) {
  ...
  function getEntropyTypeStr(entropy) {
    ...
    if (entropy.base.asInt == 52) {
      ...
      if (dupes.length > 0) {
        ...
        var msg =
          dupes.length + " " + dupeWord + ": " + dupes.slice(0, 3).join(" ");
        if (dupes.length > 3) {
          msg += "...";
        }
        cardDetail.push(msg);
  ...
  var networks = [
    {
      name: "777 - Jackpot",
      onSelect: function () {
        network = libs.bitcoin.networks.jackpot;
        setHdCoin(833);
      },
    },
    {
      name: "AZR - Azzure",
      onSelect: function () {
        network = libs.bitcoin.networks.azr;
        setHdCoin(835);
      },
    },
    {
      name: "BECN - Beacon",
      onSelect: function () {
        network = libs.bitcoin.networks.becn;
        setHdCoin(841);
      },
    },
    {
      name: "BIR - Birake",
      onSelect: function () {
        network = libs.bitcoin.networks.birake;
        setHdCoin(848);
      },
    },
    {
    ...
    },
    {
      name: "PNY - Peony",
      onSelect: function () {
        network = libs.bitcoin.networks.pny;
        setHdCoin(840);
      },
    },
    {
    ...
    },
    {
      name: "TRTT - Trittium",
      onSelect: function () {
        network = libs.bitcoin.networks.trtt;
        setHdCoin(838);
      },
    },
    {
  ...
  function displayBip44InfoForELA() {
    ...
    // Calculate the account extended keys
    var accountXprv = libs.elastosjs.getAccountExtendedPrivateKey(
      seed,
      coin,
      account
    );
    var accountXpub = libs.elastosjs.getAccountExtendedPublicKey(
      seed,
      coin,
      account
    );
  ...
  function displayBip32InfoForELA() {
    ...
    DOM.extendedPrivKey.val(
      libs.elastosjs.getBip32ExtendedPrivateKey(seed, coin, account, change)
    );
    DOM.extendedPubKey.val(
      libs.elastosjs.getBip32ExtendedPublicKey(seed, coin, account, change)
    );
  ...
  function calcAddressForELA(seed, coin, account, change, index) {
    ...
    var publicKey = libs.elastosjs.getDerivedPublicKey(
      libs.elastosjs.getMasterPublicKey(seed),
      change,
      index
    );
    return {
      privateKey: libs.elastosjs.getDerivedPrivateKey(
        seed,
        coin,
        account,
        change,
